...
TupleValueDenseTube::TupleValueDenseTube(const TupleValueSparseTube& tupleValueSparseTube, const unsigned int nbOfHyperplanes) : tube(nbOfHyperplanes)
{
  for (const pair<unsigned int, double>& keyValue : tupleValueSparseTube)
    {
      tube[keyValue.first] = keyValue.second;
    }
}
...
void TupleValueDenseTube::setSum(double& sum) const
{
