...
const bool TupleValueTrie::setTupleValues(const vector<vector<unsigned int>>::const_iterator dimensionIt, const double value, const unsigned int sizeThreshold, const unsigned int lastDimensionCardinality)
{
  ...
  for (const unsigned int element : *dimensionIt)
	...
	{
	  TupleValueDenseTube* newHyperplane = new TupleValueDenseTube(static_cast<TupleValueSparseTube&>(*hyperplane), lastDimensionCardinality);
	  delete hyperplane;
...
void TupleValueTrie::setSum(double& sum) const
{
  for (const AbstractTupleValueData* hyperplane : hyperplanes)
    {
      hyperplane->setSum(sum);
    }
}
