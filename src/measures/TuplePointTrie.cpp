...
const bool TuplePointTrie::setTuplePoints(const vector<vector<unsigned int>>::const_iterator dimensionIt, const pair<double, double>& point, const unsigned int sizeThreshold, const unsigned int lastDimensionCardinality)
{
  ...
  for (const unsigned int element : *dimensionIt)
	...
	{
	  TuplePointDenseTube* newHyperplane = new TuplePointDenseTube(static_cast<TuplePointSparseTube&>(*hyperplane), lastDimensionCardinality);
	  delete hyperplane;
...
void TuplePointTrie::minCoordinates(double& minX, double& minY) const
{
  for (const AbstractTuplePointData* hyperplane : hyperplanes)
    {
      hyperplane->minCoordinates(minX, minY);
    }
}
